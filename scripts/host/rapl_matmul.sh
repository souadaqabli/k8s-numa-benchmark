#!/bin/bash
# Simplified copy of the 1st Config RAPL campaign script, adapted for the
# 4 matmul scenarios at N=4 only.
#
...
declare -A SCENARIO_FILES=(
    ["baseline"]="deployments/matmul/baseline.yaml"
    ["extreme"]="deployments/matmul/extreme.yaml"
    ["cross-numa"]="deployments/matmul/cross-numa.yaml"
    ["extreme-cross"]="deployments/matmul/extreme-cross.yaml"
    ["native"]="deployments/matmul/native.yaml"
)
...
# Separate file, never mixed with the existing CSVs
RESULTS_FILE="${RESULTS_DIR}/RAPL/matmul_energy_results.csv"
...
echo "========================================================"
echo " MATMUL CAMPAIGN - N4 - 5 SCENARIOS"
echo "========================================================"
...
for SCENARIO in "${SCENARIOS[@]}"; do
    ...
    } > "${RESULTS_DIR}/numastat/numastat_matmul_${CORE}_${SCENARIO}.txt" 2>&1
    ...
    if [ -f /tmp/rapl_results_local.txt ]; then
        ...
        echo "matmul,$CORE,$SCENARIO,$DURATION,$TOTAL_EN,$PKG0_EN,$PKG1_EN,$DRAM0_EN,$DRAM1_EN" >> "$RESULTS_FILE"
    else
...
echo ""
echo "========================================================"
echo " MATMUL N4 CAMPAIGN FINISHED"
