...
def extract_arrays(df_median, pattern):
    ...
    for cat in CATEGORIES:
        if cat in sub.index:
            ...
            energy_arr.append([row['pkg0'] / 1_000_000, row['pkg1'] / 1_000_000,
                                row['dram0'] / 1_000_000, row['dram1'] / 1_000_000])
        else:
            time_arr.append(0)
...
def create_figure(time_seq, time_rand, energy_seq, energy_rand, filename):
    ...
    # --- GRAPHE 2 : ÉNERGIE TOTALE (JOULES) ---
    ...
    ax2.set_title('N=12 - Total Energy Distribution (PKG + DRAM)', fontsize=14)
...
if __name__ == "__main__":
    print("=== Génération Énergie Totale (PKG+DRAM) - 4 scénarios N12 + Native N12 ===")
    ...
    create_figure(time_seq, time_rand, energy_seq, energy_rand,
                  "analysis/energy_total_n12_native.png")
