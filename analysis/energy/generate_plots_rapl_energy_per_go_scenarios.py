...
def load_global_rapl_data(csv_path):
    ...
    with open(csv_path, 'r') as f:
        ...
        for row in reader:
            ...
            # --- Normalisation par Go traité ---
            per_go = [pkg0_j / GB_PROCESSED, pkg1_j / GB_PROCESSED,
                      dram0_j / GB_PROCESSED, dram1_j / GB_PROCESSED]
...
def create_figure(time_seq, time_rand, ener_seq, ener_rand, title_prefix, filename):
    ...
    ax2.set_ylabel(f'Energy per GB (Joules/GB, {GB_PROCESSED} GB processed)', fontweight='bold')
    ax2.set_title(f'{title_prefix} - Energy Distribution per GB', fontsize=14)
    ax2.grid(axis='y', linestyle='--', alpha=0.7)

    for i in range(len(x)):
        ax2.text(x[i] - width/2, bottom_seq[i] + (max(bottom_rand)*0.02), f"{bottom_seq[i]:.1f}", ha='center', va='bottom', fontsize=10)
        ax2.text(x[i] + width/2, bottom_rand[i] + (max(bottom_rand)*0.02), f"{bottom_rand[i]:.1f}", ha='center', va='bottom', fontsize=10)
...
print("=== Génération Energie/Go - 4 scénarios NUMA (Work-Bound) ===")
...
if data_work:
    ...
    create_figure(wb_time_seq, wb_time_rand, wb_ener_seq, wb_ener_rand,
                  "WORK-BOUND Mode", "analysis/energy_per_go_workbound_scenarios.png")
