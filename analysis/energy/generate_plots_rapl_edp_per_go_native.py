...
def load_global_rapl_data(csv_path):
    ...
    with open(csv_path, 'r') as f:
        ...
        for row in reader:
            ...
            # --- EDP/Go = (Energie x Temps) / Go traité (volume système = pods x 120 GiB) ---
            gb = GB_PROCESSED[scen]
            edp_per_go = [pkg0_j * time_val / gb, pkg1_j * time_val / gb,
                          dram0_j * time_val / gb, dram1_j * time_val / gb]
...
def create_figure(time_seq, time_rand, edp_seq, edp_rand, title_prefix, filename):
    ...
    ax2.set_ylabel('EDP per GB (J·s/GB)', fontweight='bold')
    ax2.set_title(f'{title_prefix} - EDP Distribution per GB', fontsize=14)
    ax2.grid(axis='y', linestyle='--', alpha=0.7)

    for i in range(len(x)):
        if bottom_seq[i] > 0:
            ax2.text(x[i] - width/2, bottom_seq[i] + (max(bottom_rand)*0.02), f"{bottom_seq[i]:.0f}", ha='center', va='bottom', fontsize=9)
        if bottom_rand[i] > 0:
            ax2.text(x[i] + width/2, bottom_rand[i] + (max(bottom_rand)*0.02), f"{bottom_rand[i]:.0f}", ha='center', va='bottom', fontsize=9)
...
print("=== Génération EDP/Go - 4 scénarios NUMA + Native N4/N16 (Work-Bound) ===")
...
if data_work:
    ...
    create_figure(wb_time_seq, wb_time_rand, wb_edp_seq, wb_edp_rand,
                  "WORK-BOUND Mode", "analysis/edp_per_go_workbound_native.png")
