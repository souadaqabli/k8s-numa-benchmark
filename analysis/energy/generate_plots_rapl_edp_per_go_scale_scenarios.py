...
def plot_scaling_edp_per_go(rapl_csv):
    # 1. Load Data
    ...
    # N20 ignoré pour le moment, et on ne garde que les 4 scénarios NUMA (pas native)
    df_rapl = df_rapl[(df_rapl['n_cores'] != 20) & (df_rapl['scen_type'].isin(SCENARIOS))]
    ...
    # 4. MEDIAN CALCULATION (plusieurs runs par config) - inclut aussi 'time'
    ...
    # 6. Calcul de l'EDP par Go : Energie (J) x Temps (s) / Go traité
    df_rapl_median['edp_j_s_per_gb'] = (
        (df_rapl_median['true_total_uj'] / 1_000_000) * df_rapl_median['time']
        / df_rapl_median['gb_processed']
    )
    ...
    sns.lineplot(
        data=df_seq, x='n_cores', y='edp_j_s_per_gb',
        hue='scen_type', marker='o', palette=custom_palette,
        ax=ax1, linewidth=2.5, markersize=8
    )
    ax1.set_title('Sequential Access - EDP per GB Scalability', fontsize=14, fontweight='bold')
    ax1.set_xlabel('Number of CPU Cores (Pods)', fontsize=12)
    ax1.set_ylabel('EDP Cost per GB (J·s/GB)', fontsize=12, fontweight='bold')
    ax1.grid(True, linestyle='--', alpha=0.7)

    # ---- RIGHT Plot (Random) ----
    df_rand = df_rapl_median[df_rapl_median['pattern'] == 'rand']
    sns.lineplot(
        data=df_rand, x='n_cores', y='edp_j_s_per_gb',
        hue='scen_type', marker='s', palette=custom_palette,  # square markers for rand
        ax=ax2, linewidth=2.5, markersize=8
    )
    ax2.set_title('Random Access - EDP per GB Scalability', fontsize=14, fontweight='bold')
    ax2.set_xlabel('Number of CPU Cores (Pods)', fontsize=12)
    ax2.set_ylabel('EDP Cost per GB (J·s/GB)', fontsize=12, fontweight='bold')
    ...
    plt.savefig('analysis/scaling_edp_per_go_scenarios.png', dpi=300, bbox_inches='tight')
    print("[SUCCESS] Plot generated: analysis/scaling_edp_per_go_scenarios.png")

# Execution
if __name__ == "__main__":
    plot_scaling_edp_per_go("results/RAPL/scalability_energy_results_complete.csv")
